...
def df_real(spec: Tensor, coefs: Tensor) -> Tensor:
    ...
    Args:
        spec (real-valued Tensor): Spectrogram of shape [B, C, N, F, 2].
        coefs (real-valued Tensor): Coefficients of shape [B, C, N, F, 2].

    Returns:
        spec (real-valued Tensor): Filtered Spectrogram of shape [B, C, F, 2]
    """
    b, c, _, f, _ = spec.shape
    real_part = (spec[..., 0] * coefs[..., 0]).sum(dim=2) - (spec[..., 1] * coefs[..., 1]).sum(dim=2)
    imag_part = (spec[..., 0] * coefs[..., 1]).sum(dim=2) + (spec[..., 1] * coefs[..., 0]).sum(dim=2)
...
class DF(MultiFrameModule):
    ...
    def __init__(self, freq_bins: int, num_freqs: int, frame_size: int, lookahead: int = 0, conj: bool = False):
        ...
        self.coefs_buffer = CyclicBuffer(
            shape=[1, 5, 1, num_freqs, 2],
            time_steps=1,
            delay_frames=2,
            time_dim=2
        )
    ...
    def initial_state(
        ...
        if state is not None:
            state = state.reshape(-1).to(device=device, dtype=dtype)
            if state.numel() != self.state_size():
                raise ValueError(f"Initial state size mismatch in DF: expected {self.state_size()}, got {state.numel()}")
            return state
        return torch.cat(
            [
                self.coefs_buffer.initial_state(device=device, dtype=dtype),
                self.spec_buffer.initial_state(device=device, dtype=dtype),
            ],
            dim=0,
        )
    ...
    def forward(
        ...
        if state is None:
            coefs_buffered = self.coefs_buffer(coefs)
        else:
            coefs_buffered, coefs_state_out, offset = self.coefs_buffer(coefs, state=state, offset=offset)
            state_out_chunks.append(coefs_state_out)
        coefs_buffered = coefs_buffered.permute(0, 2, 1, 3, 4)
        if state is None:
            full_spec_buffered = self.spec_buffer(spec)
        else:
            full_spec_buffered, spec_state_out, offset = self.spec_buffer(spec, state=state, offset=offset)
            state_out_chunks.append(spec_state_out)
        spec_f = full_spec_buffered[..., :self.num_freqs, :]
        full_spec_frame = full_spec_buffered[:, :, 2:3]
        # spec_f & coefs_buffered shapes: [B, C, N, F, 2]
        spec_df = df_real(spec_f, coefs_buffered)
        # spec_f [B, C, F, 2]
        spec_df = spec_df.unsqueeze(1)
        spec_df_enh = torch.cat([spec_df, full_spec_frame[:, :, :, self.num_freqs:]], dim=3)
        if state is None:
            return spec_df_enh
        state_out = torch.cat(state_out_chunks, dim=0)
        return spec_df_enh, state_out, offset
