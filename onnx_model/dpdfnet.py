...
from .layers import Stft, Istft, ErbNorm, \
    ...
    ConvTranspose2dNormAct, Mask, SpecNorm, SubPixelConv2dNormAct, DPRNN, CyclicBuffer
from model.utils import as_real, to_db, get_wnorm, vorbis_window, erb_filter_banks
from .utils import get_pow
...
class Encoder(Module):
    ...
    def forward(
        ...
        e0 = self.erb_conv0(feat_erb_buffered)  # [B, C, T, F]
        ...
        if state is None or not hasattr(self.dprnn_df, "state_size"):
            ...
            c1, dprnn_df_state_out, offset = self.dprnn_df(c1, state=state, offset=offset)
            state_out_chunks.append(dprnn_df_state_out)
        cemb = c1.permute(0, 2, 3, 1).flatten(1)  # [B, -1]
        cemb = self.df_fc_emb(cemb)  # [B, C * F_erb/4]
        emb = e3_dprnn.permute(0, 2, 3, 1).flatten(1)  # [B, C * F_erb/4]
        emb = self.combine(emb, cemb)
        if state is None:
            emb = self.emb_gru(emb)  # [B, T, -1]
        else:
            emb, emb_gru_state_out, offset = self.emb_gru(emb, state=state, offset=offset)  # [B, T, -1]
...
class DPDFNet(Module):
    def __init__(
        ...
        self.erb_norm = ErbNorm(num_feat=self.nb_erb,
                                alpha=alpha_norm,
                                dynamic_var=erb_dynamic_var,
                                stateful=norm_stateful
                                )
        self.spec_norm = SpecNorm(num_feat=self.nb_df,
                                  alpha=alpha_norm,
                                  stateful=norm_stateful
                                  )
    ...
    def initial_state(
        ...
        if state is not None:
            state = state.reshape(-1)
            if state.numel() != self.state_size():
                raise ValueError(f"Initial state size mismatch in DPDFNet: expected {self.state_size()}, got {state.numel()}")
            return state
        chunks = [
    ...
    def _feature_extraction(
        ...
        """Forward method of DeepFilterNet2.
                ...
                """
        feat_erb = get_pow(spec) @ self.erb_fb    # (B, T, E)
        if self.erb_to_db:
...
if __name__ == '__main__':
    waveform_16k, sr = soundfile.read("./noisy_16khz.wav")
    reference_path = "./enhanced_ref_16khz.wav"
    output_path = "./enhanced_flat_buffer_16khz.wav"
    model = DPDFNet(
    ...
    # load PT weights
    state_dict = torch.load('./model_zoo/checkpoints/dpdfnet2.pth', weights_only=True, map_location='cpu')
    stream_state_dict = correct_state_dict(state_dict)
    model.load_state_dict(stream_state_dict, strict=True)
    model.eval()

    spec = model.apply_stft(torch.tensor(waveform_16k, dtype=torch.float32)[None, :]) # [B=1, T, F]
    ...
    waveform_16k_e = model.apply_istft(spec_e).detach().cpu().numpy().flatten()

    soundfile.write(output_path, waveform_16k_e, sr)
